...
const RestaurantDishTable = ({restaurantData}) => {
    return (
        ...
        <table className="auto w-full table-fixed mt-3">
            ...
            <tbody>
                {restaurantData?.dish?.map((dish,index)=>{
                    return(
                    <tr key={index} className="xl:text-xs text-sm h-10 border-2 border-r bg-[#fafafa]">
                      ...
                      <td className="border border-slate-700">
                        ...
                        <Link href={`edit/${dish.uuid}`} className="p-2 lg:p-1  bg-blue-600 text-white">Edit</Link>
                        <Link href={`delete/${dish.uuid}`} className="p-2 lg:p-1 bg-red-600 text-white">Delete</Link>
                        </div>
                    </td>     
...
export default RestaurantDishTable